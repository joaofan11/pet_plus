...
const findUserByEmail = async (email) => {
  const { rows } = await db.query(
    `SELECT id, name, email, phone, photo_url AS "photoUrl", auth_id 
...
const createUser = async (name, email, phone, authId, photoUrl) => {
  const { rows } = await db.query(
    `INSERT INTO users (name, email, phone, auth_id, photo_url) 
     VALUES ($1, $2, $3, $4, $5) 
     RETURNING id, name, email, phone, photo_url AS "photoUrl"`,
    [name, email, phone, authId, photoUrl]
  );
  return rows[0];
};
